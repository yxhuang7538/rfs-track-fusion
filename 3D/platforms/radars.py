...
class Radar:
    ...
    def config(self, cfg_path: str) -> None:
        ...
        self.theta_min = cfg['radar_theta_min'] * np.pi / 180     # 俯仰角范围 弧度
        ...
        for traj in trajs:  # 取k时刻所有目标轨迹点
            ...
            for state_k in traj:
                if np.random.rand() <= self.p_d:    # 目标存活，被检测到
                    z = self.cart2pol(state_k) + np.random.multivariate_normal(np.zeros(3), self.R)
                    meas.append(z)
            # 加入杂波
            for _ in range(int(kz.rvs())):
                ...
                meas.append(np.array([r, phi, theta]))  # 距离 方位 俯仰
            all_meas.append(meas)   # k时刻所有量测
        self.all_meas = all_meas
        return all_meas
    
    def cart2pol(self, state: np.ndarray) -> np.ndarray:
        r = np.linalg.norm(self.pos - state[[0, 3, 6]])
        phi = np.arctan2(state[3] - self.pos[1], state[0] - self.pos[0])
        theta = np.arctan2(np.linalg.norm(self.pos[[0, 1]] - state[[0, 3]]), state[6] - self.pos[2])

        return np.array([r, phi, theta])

    def pol2cart(self, z: np.ndarray) -> np.ndarray:
        x_ = z[0] * np.cos(z[1]) * np.sin(z[2])
        y_ = z[0] * np.sin(z[1]) * np.sin(z[2])
        z_ = z[0] * np.cos(z[2])
        tmp = np.array([x_, y_, z_])
        return tmp
